...
def run_experiments(experiments, threads, *, n, sigma_s, sigma_e, sigma_eps, sigma_h_s, sigma_h_e, t):
    ...
    with Pool(threads) as p:
        ...
        args = [{'id': i, 'n': n, 'rtSigma': rtSigma, 'sigma_s': sigma_s, 'sigma_e': sigma_e, 'sigma_eps': sigma_eps, 'D_e1': D_e1, 'D_v': D_v, 't': t} for i in range(experiments)]
        fast_results = {}
    ...
    print()
    print(f'Theoretical: {theoretical_det}')
    print(f'Slow: {ln(sum(slow_results.values())/experiments)}')
    print(f'Fast: {ln(sum(fast_results.values())/experiments)}')
    print(f'Difference: {ln(sum(abs(slow_results[id] - fast_results[id]) for id in slow_results)/experiments)}')
